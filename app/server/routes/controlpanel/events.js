			...
			DB.canIeditThis("events", {"permalink":params2[0]}, req.session.passport.user, function (result) {
				if (result) {
					...
					if (params2.length==1) {
						var subsez = "public";
						var msg = [];
						res.render('forms/event_public', {locals: {form:"event_public", title:result.title+": "+__("Main data"), sez:sez, subsez:subsez, countries: CT, result:result, msg:msg,Fnc:Fnc}, user : req.session.passport.user });
					} else if (params2[1]=="mainimage") {
						var subsez = "mainimage";
						var msg = [];
						res.render('forms/event_mainimage', {locals: {title:result.title+": "+__("Main image"), sez:sez, subsez:subsez, result:result, msg:msg,Fnc:Fnc}, user : req.session.passport.user });
					} else if (params2[1]=="partners") {
						var subsez = "partners";
						var msg = [];
						res.render('forms/event_partners', {locals: {title:result.title+": "+__("Partners"), sez:sez, subsez:subsez, result:result, msg:msg,Fnc:Fnc}, user : req.session.passport.user });
					} else if (params2[1]=="performances") {
						var subsez = "performances";
						var msg = [];
						res.render('forms/event_performances', {locals: {title:result.title+": "+__("Performances"), sez:sez, subsez:subsez, result:result, msg:msg,Fnc:Fnc}, user : req.session.passport.user });
					} else if (params2[1]=="gallery") {
						var subsez = "gallery";
						var msg = [];
						res.render('forms/event_gallery', {locals: {title:result.title+": "+__("Gallery"), sez:sez, subsez:subsez, result:result, msg:msg,Fnc:Fnc}, user : req.session.passport.user });
					} else if (params2[1]=="settings") {
						var subsez = "settings";
						var msg = [];
						res.render('forms/event_settings', {locals: {title:result.title+": "+__("Settings"), sez:sez, subsez:subsez, result:result, msg:msg,Fnc:Fnc}, user : req.session.passport.user });
					}
				} else {
					  		...
					  		DB.users.save(newItem, {safe:true}, function(e, success) {
							  	DB.users.findOne({_id:result._id},function(e, result3) {
							  		...
							  		DB.updateEventRel(result._id, function(success) {
										res.render('forms/'+form, {locals: {form:form, title:result3.title+": "+titles[subsez], sez:sez, subsez:subsez, countries: CT, result:result3, msg:{c:[{m:m}]},Fnc:Fnc}, user : req.session.passport.user });
							  		});
						  		});
					  		});
						}
					} else {
						res.render('forms/'+form, {locals: {form:"user_public", title:result.title+": "+titles[subsez], sez:sez, subsez:subsez, countries: CT, result:req.body, msg:{e:errors},Fnc:Fnc}, user : req.session.passport.user });
					}
		  		});
			} else {
				res.render('forms/cannotedit', {locals: {}, user : req.session.passport.user });

<div class="cntTitPalette">
	...
	<div class="right new">			
	<? if($_POST['public'][0]==1){ ?>
		<a href="/<? echo($this->community->getPermalinkStr($_POST['uid'],"soggetti")."/iflxerplaylists/".$this->community->getPermalinkStr($_GET['playlistsIflxerid'],"playlist")); ?>/" class="right"><? echo($this->getLabel("Mostra")); ?></a>
	<? } ?>	&nbsp;		
	</div>
	<br class="myClear" />
</div>
<form method="post" name="playlist_edit" id="playlist_edit" action="<? echo($_SERVER['PHP_SELF']); ?>?<? echo($_SERVER['QUERY_STRING']); ?>" enctype="multipart/form-data">
	<div class="boxFormFull">
		...
		<input type="text" name="title" id="title" class="textfields" value="<? echo(isset($_POST['title']) ? $_POST['title'] : ""); ?>" maxlength="255" onfocus="clearError(this,'msgErrorCnt_title')" />
		<br class="myClear" />
	</div>
	<div class="boxFormFull">
		...
		<div id="lista">	
		<?
			$_GET['id'] = $_GET['playlistsIflxerid'];
